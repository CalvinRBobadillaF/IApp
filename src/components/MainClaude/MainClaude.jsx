...
<div className="title-div-claude"> 
    ...
    {models ? <DropDown /> : null}
                ...
                <img src={assets.user_icon} alt="user" onClick={() => setOpenModal(!openModal)} className="profile-image" />
            </div>
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 6.75 22.5 12l-5.25 5.25m-10.5 0L1.5 12l5.25-5.25m7.5-3-4.5 16.5" />
                    ...
                    <div className="result-claude">
                        {currentChat.messages.map((msg, i) => (
                            <div key={i}
                                className={msg.role === "user" ? "result-title-gpt user-message-claude" : "result-data-gpt ai-message-claude"}
                            >
                                <img
                                    src={msg.role === "user" ? assets.user_icon : assets.claude_icon}
                                    alt=""
                                />

                                {msg.role === "model" && i === currentChat.messages.length - 1 && loading ? (
                                    resultData.length === 0 ? (
                                        <div className="loader-claude"><hr /><hr /><hr /></div>
                                    ) : (
                        ...
                        {loading && currentChat.messages.at(-1)?.role === "user" && (
                                                <div className="result-data ai-message">
                                                    ...
                                                    <div className="thinking"> ⚙️Coding...</div>
                                                </div>
                                            )}
                    </div>
